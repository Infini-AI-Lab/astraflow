#!/bin/bash
set -euo pipefail
# All-in-one launcher for AstraFlow training with the DEEPDIVE RECURSIVE-AGENT
# workflow (Qwen3-4B-Instruct-2507, M2PO, platoon port).
#
# Workflow: deepdive_recursive. Agent emits one
# <thought>...</thought><python>...</python> per turn. Python sandbox
# exposes async tools: search_web (CMU RAG), launch_subagent, finish.
# Reward: LLM judge for root (vs ground_truth) and for sub-agents.
#
# DeepDive task data auto-downloaded from HF (zai-org/DeepDive) on first
# run; cache lands at astraflow/core/workflow/impl/deepdive/deepdive_*.jsonl.
#
# External services required:
#   - Fireworks API:   ~/.fireworks_key  (FIREWORKS_API_KEY for LLM judge)
#   - CMU RAG search:  ~/.cmu_search_key (CMU_SEARCH_API_KEY for search_web)
#
...
#
# Usage:
#   bash examples/deepdive/qwen3-4b-recursive/scripts/run_qwen3-4b-recursive.sh
...
# Source API keys for the LLM judge and the CMU RAG search. Both files are
# gitignored locally (~/.fireworks_key, ~/.cmu_search_key). The trainer needs
# both visible in its env; we export them with `set -a` so child processes
# inherit them.
set -a
[[ -f "${HOME}/.fireworks_key" ]] && source "${HOME}/.fireworks_key"
[[ -f "${HOME}/.cmu_search_key" ]] && source "${HOME}/.cmu_search_key"
set +a
if [[ -z "${FIREWORKS_API_KEY:-}" ]]; then
  echo "WARNING: FIREWORKS_API_KEY not set — LLM judge calls will fail" >&2
fi
if [[ -z "${CMU_SEARCH_API_KEY:-}" ]]; then
  echo "WARNING: CMU_SEARCH_API_KEY not set — search_web calls will fail" >&2
fi
...
# =============================================================================
echo "=== AstraFlow (Qwen3-4B, DeepDive, M2PO, ctx40k, TCP, RECURSIVE AGENT) ==="
