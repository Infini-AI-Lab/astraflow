#!/bin/bash
set -euo pipefail
# [3/3] Launch Trainer (TCP, sender_agent on local_rank 0)
#
# Usage (terminal 3, after RaaS and AstraFlow are ready):
#   bash examples/code-multi-agent/qwen3-8b-single-agent-m2po-full/scripts/3_trainer.sh
...
export CUDA_VISIBLE_DEVICES="${TRAINER_GPUS:-4,5,6,7}"
TRAINER_NPROC="$(echo "${CUDA_VISIBLE_DEVICES}" | awk -F',' '{print NF}')"
...
echo "GPUs              : ${CUDA_VISIBLE_DEVICES} (FSDP dp${TRAINER_NPROC})"
