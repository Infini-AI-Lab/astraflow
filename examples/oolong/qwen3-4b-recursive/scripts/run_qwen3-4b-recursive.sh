#!/bin/bash
set -euo pipefail
# All-in-one launcher for AstraFlow training with the RECURSIVE-AGENT
# workflow on TextCraft (Qwen3-4B-Instruct-2507, M2PO, platoon port).
#
# Workflow: recursive_agent.  Agent emits one <action type="...">{JSON}</action>
# per turn (craft / get_info / view_inventory / spawn / finish).  Spawn
# fans out up to 4 sub-agents in parallel via asyncio.gather, each
# inheriting the parent's inventory by reference.  Tree-structured
# trajectory flattened to one trajectory with N sequences sharing the
# env-based team reward.
#
# TextCraft task data is synthesized locally on first run from the
# bundled Minecraft recipe DB (no HF download required).  Files land at
# astraflow/core/workflow/impl/textcraft/textcraft_{train,val}.jsonl.
#
...
# =============================================================================
echo "=== AstraFlow (Qwen3-4B, TextCraft, M2PO, ctx32k, TCP, RECURSIVE AGENT) ==="
