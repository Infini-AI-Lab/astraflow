#!/bin/bash
set -euo pipefail
# [4/4] Launch Trainer for model1 -- verifier (TCP, sender_agent on local_rank 0)
#
# Usage (terminal, after AstraFlow and RaaS are ready):
#   bash examples/math-multi-agent/qwen3-8b-actor-verifier-m2po-delta-no-ds-deepscaler-data/scripts/4_trainer_model1.sh
...
TRAINER1_NPROC="$(echo "${CUDA_VISIBLE_DEVICES}" | awk -F',' '{print NF}')"
...
echo "=== Trainer model1 -- verifier (TCP) ==="
echo "Experiment config   : ${EXPERIMENT_CONFIG}"
echo "GPUs                : ${CUDA_VISIBLE_DEVICES} (FSDP dp${TRAINER1_NPROC})"
