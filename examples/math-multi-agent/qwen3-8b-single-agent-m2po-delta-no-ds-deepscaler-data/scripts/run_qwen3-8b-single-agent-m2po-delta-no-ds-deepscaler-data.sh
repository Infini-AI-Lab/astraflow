#!/bin/bash
set -euo pipefail
# All-in-one launcher for AstraFlow v2 math training (NO dynamic-sampling filter).
#
# Same as qwen3-8b-single-agent-m2po-delta-deepscaler-data but with the buffer's filter_function
...
#   3. Trainer model0         (TRAINER_MODEL0_GPUS)
#
# Usage:
#   bash examples/math-multi-agent/qwen3-8b-single-agent-m2po-delta-no-ds-deepscaler-data/scripts/run_qwen3-8b-single-agent-m2po-delta-no-ds-deepscaler-data.sh
...
# =============================================================================
echo "=== AstraFlow v2 (Qwen3-8B single-agent, math-multi-agent, M2PO, ctx16k, TCP delta, no-filter) ==="
