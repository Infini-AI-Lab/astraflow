#!/bin/bash
# AMD/ROCm launcher for the llama3-8b-instruct-m2po-delta recipe (MI300/MI325, gfx942).
#
...
#   bash examples/math/llama3-8b-instruct-m2po-delta/scripts/run_llama3-8b-instruct-m2po-delta_amd.sh
# Under Slurm (pyxis-free; uses the node's docker daemon):
#   srun --gres=gpu:8 --nodes=1 --ntasks=1 bash examples/math/.../run_llama3-8b-instruct-m2po-delta_amd.sh
#
# Env knobs:
#   CONTAINER_IMAGE  docker image tag (default: astraflow:rocm)
#   SMOKE_STEPS      cap total_train_steps for a quick bring-up check
#   HF_TOKEN         HuggingFace token (Llama-3-8B-Instruct is a gated download)
...
bash examples/math/llama3-8b-instruct-m2po-delta/scripts/run_llama3-8b-instruct-m2po-delta.sh ${OVERRIDES[*]}
INNER_EOF
