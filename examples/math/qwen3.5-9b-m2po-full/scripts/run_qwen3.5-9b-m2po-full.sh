#!/bin/bash
set -euo pipefail
# All-in-one launcher for AstraFlow v2 math training (Qwen3.5-9B, M2PO, TCP).
#
...
#
# Requires: transformers>=5.8 (+ flash-linear-attention for training),
#           SGLang main (qwen3_5 model). See yaml/raas.yaml for the backend note.
#
# Usage:
#   bash examples/math/qwen3.5-9b-m2po-full/scripts/run_qwen3.5-9b-m2po-full.sh
...
# This box's PCIe P2P is broken: cudaDeviceCanAccessPeer reports True but the
# DMA never completes, so any NCCL group that picks `via P2P/IPC` hangs forever
# (verified with a minimal 2-rank all_reduce repro). 4-GPU groups happen to pick
# SHM and work by luck; forcing SHM makes that deterministic at no cost, since
# nothing here was using P2P anyway.
...
# =============================================================================
echo "=== AstraFlow (Qwen3.5-9B, math, M2PO, ctx8k, TCP full) ==="
