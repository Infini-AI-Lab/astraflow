# Qwen3.5-9B — Math RL (M2PO)

Text-only math RL on **Qwen/Qwen3.5-9B** with **M2PO**, context 8k, lr 5e-6,
DeepScaleR data, `math_verify` reward.

Qwen3.5-9B is a **hybrid Gated-DeltaNet + attention multimodal** checkpoint
...
|---|---|
| `qwen3.5-9b-m2po-full`  | full (push the whole model each sync) |
| `qwen3.5-9b-m2po-delta` | delta (push only changed weights) |
...
# delta variant:
bash examples/math/qwen3.5-9b-m2po-delta/scripts/run_qwen3.5-9b-m2po-delta.sh
```
...
Qwen3.5-9B-full, overall metrics across the eval suite:
