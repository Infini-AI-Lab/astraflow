#!/bin/bash
set -euo pipefail
# All-in-one launcher for AstraFlow v2 math training with the SPAWN
# sub-agents workflow (Qwen3-8B, M2PO, TCP, offline data).
#
# The rollout/eval workflow is spawn_rlvr: the main agent may emit a single
# <spawn>{"tasks": [...]}</spawn> block per trajectory; up to 4 sub-agents
# run in parallel against the same RaaS pool; their outputs are spliced
# back into the main context.  All N+1 sequences share the team reward.
#
# Launches 3 processes:
#   1. AstraFlow HTTP service (CPU-only) — loads datasets from data-data/math
#   2. RaaS inference server  (SGLang, SERVICE_CUDA_VISIBLE_DEVICES)
#   3. Trainer model0         (math, TRAINER_MODEL0_GPUS)
#
# One-time prep (downloads ~hundreds of MB):
...
# =============================================================================
echo "=== AstraFlow v2 (Qwen3-8B, math, M2PO, ctx16k, TCP, SPAWN sub-agents) ==="
