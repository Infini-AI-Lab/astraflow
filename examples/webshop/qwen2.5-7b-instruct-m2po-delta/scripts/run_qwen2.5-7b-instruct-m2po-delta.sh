#!/bin/bash
set -euo pipefail
# All-in-one launcher for Webshop AstraFlow v2 training.
#
# Launches 4 processes:
#   0. AgentBench Webshop Server (AGENTBENCH_PORT must match YAML workflow_spec.task_server_url)
#   1. AstraFlow HTTP service
#   2. RaaS inference server (SGLang, SERVICE_CUDA_VISIBLE_DEVICES)
#   3. Trainer              (TRAINER_MODEL0_GPUS)
#
# Usage:
...
export ASTRAFLOW_CLEANUP_PORTS="${WEIGHT_TRANSFER_HTTP_PORT_MODEL0} 21000"
...
# =============================================================================
echo "=== Webshop AstraFlow v2 (TCP delta weight transfer) ==="
echo "Experiment config : ${EXPERIMENT_CONFIG}"
echo "RaaS config       : ${RAAS_CONFIG}"
echo "RaaS GPUs         : ${SERVICE_CUDA_VISIBLE_DEVICES}"
echo "Trainer GPUs      : ${TRAINER_MODEL0_GPUS} (FSDP dp${TRAINER0_NPROC})"
echo "RaaS port         : ${RAAS_PORT}"
echo "AstraFlow port    : ${ASTRAFLOW_PORT}"
echo "Sender HTTP       : ${WEIGHT_TRANSFER_HTTP_PORT_MODEL0}"
...
# =============================================================================
echo "[0/4] Starting AgentBench Webshop Server..."
