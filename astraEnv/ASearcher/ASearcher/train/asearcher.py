...
try:
    ...
    from areal.api.cli_args import GRPOConfig, load_expr_config  # type: ignore
    from areal.utils.hf_utils import load_hf_tokenizer  # type: ignore

from astraEnv.ASearcher.ASearcher.train.prompts import (
    INVALID_PROMPT,
    SEARCH_ACCESS_PROMPT_TEMPLATE,
    SEARCH_ONLY_PROMPT_TEMPLATE,
    VALID_PROMPT,
)
from astraEnv.ASearcher.ASearcher.train.search_agent import SearchAgent
from astraEnv.ASearcher.ASearcher.utils.rewards import correct_format_fn
from astraEnv.ASearcher.ASearcher.utils.search_tool import SearchToolBox
...
class ASearcherWorkflow(RolloutWorkflow):
    ...
    async def arun_episode(self, engine, data):
        ...
        for key in ["query_id", "id", "qid"]:
            ...
            if qid is not None:
                break
        qid = str(qid) if qid is not None else uuid.uuid4().hex

        # Decide whether to dump this rollout (1/128 probability).
        _should_dump = self.dump_dir is not None and random.random() < 1 / 128
        ...
        ground_truth, scores, stats = None, [], []
        ...
        # Build structured result: prompt → trajectories → sequences
        # Normalization and zero-adv filtering are handled by AstraFlow.
        ...
        for i, traj_memory in enumerate(traj_memories):
            ...
            for seq in seqs:
                ...
                res.update({k: torch.tensor([v]) for k, v in traj_stats_i.items()})
                first_llm_gen = False
        ...
        if _should_dump:
            ...
            with open(
                ...
                for i, (traj_memory, score) in enumerate(zip(traj_memories, scores)):
                    f.write(json.dumps(dict(memory=traj_memory.to_dict(), reward=score, ground_truth=ground_truth, traj_idx=i)) + "\n")
...
def main(args):
    try:
        ...
        raise RuntimeError(
            "Legacy ASearcher standalone trainer dependencies are unavailable. "
            "Use AstraFlow v2 recipe entrypoints for training."
        ) from exc
