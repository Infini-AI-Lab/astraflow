# Train Worker Design Document

## TL;DR

Follow our step-by-step code walkthrough to get started immediately!

## Motivation

The train worker provides an *algorithm-first* design philosophy built around three core
principles:
...
The codebase is structured into four distinct layers: API, backend, customization,
...
The design philosophy **discourages** implementing base classes or
infrastructure/algorithm logic in the API layer. This layer should contain only API
interfaces and utility dataclass objects. The framework prioritizes
...
are familiar with Rust or Go, the algorithm implementations are actually
...
Each recipe in `examples/` ships a `scripts/run_<name>.sh` launcher that starts the
three AstraFlow v2 processes — the AstraFlow HTTP service, the RaaS inference server,
and the trainer:
...
Every recipe under `examples/` is self-contained: a YAML config in `yaml/` plus launch
scripts in `scripts/`. Run a recipe end-to-end with its all-in-one launcher:

```bash
bash examples/math/qwen3-1.7b-m2po-full/scripts/run_qwen3-1.7b-m2po-full.sh
```

To launch the components in separate terminals (e.g. for debugging), use the numbered
scripts in the same folder instead:

```bash
bash examples/math/qwen3-1.7b-m2po-full/scripts/1_astraflow.sh        # AstraFlow HTTP service
bash examples/math/qwen3-1.7b-m2po-full/scripts/2_raas.sh             # RaaS inference server
bash examples/math/qwen3-1.7b-m2po-full/scripts/3_trainer_model0.sh   # trainer
```

Configuration parameters can be overridden for hyperparameter searches or experimental
variations — extra arguments passed to a launch script are forwarded to the trainer:

```bash
bash examples/math/qwen3-1.7b-m2po-full/scripts/run_qwen3-1.7b-m2po-full.sh \
    actor.lr=1e-4 seed=42
```
