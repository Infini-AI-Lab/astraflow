...
from astraflow.train_worker.utils import logging, name_resolve, pkg_version
from astraflow.train_worker.utils.pkg_version import is_version_less
...
class NormConfig:
    ...
    group_size: int | None = field(
        default=None, metadata={"help": "Group size for group-level normalization. Auto-set from n_samples if omitted."}
    )
...
class PPOActorConfig(TrainEngineConfig):
    ...
    group_size: int | None = field(
        default=None, metadata={"help": "Number of sequences in each group. Auto-set from n_samples if omitted."}
    )
    temperature: float | None = field(
        default=None, metadata={"help": "Temperature for importance weight calculation. Auto-set from top-level temperature if omitted."}
    )
    ppo_n_minibatches: int = field(
        default=4, metadata={"help": "Number of minibatches for each PPO update"}
    )
    filter_zero_adv_in_batch: bool = field(
        default=True,
        metadata={
            "help": "If True, drop samples whose group_reward_std == 0 from each "
            "fetched training batch on rank 0 before DP dispatch. Per-step batch "
            "size becomes variable. Independent of buffer.filter_function — both "
            "can be enabled."
        },
    )
    ...
    kl_penalty_coef: float = field(
        default=0.0,
        metadata={
            "help": "Independent KL penalty coefficient between current policy and reference policy"
        },
    )
...
class EvaluatorConfig(_Timer):
    """Configuration for evaluator scheduling."""

    eval_at_start: bool = field(
        default=True,
        metadata={
            "help": "If True, run a full evaluation pass at version=0 "
            "(before the first training step) on fresh runs. Ignored on "
            "recovery runs (start_step > 0)."
        },
    )
...
class BufferConfig:
    ...
    size: int = field(
        default=65536,
        metadata={"help": "Maximum number of examples to store in the buffer."},
    )
    debug: bool = field(
        default=False,
        metadata={"help": "If True, enable debug printing in the rollout buffer."},
    )
    filter_function: str | None = field(
        default=None,
        metadata={
            "help": "Name of the filter function from the registry. "
            "Available filters: 'keep_all', 'filter_zero_adv'. "
            "If None, uses KeepAllFilter (accepts all examples)."
        },
    )
    max_staleness: int | None = field(
        default=None,
        metadata={
            "help": "Maximum allowed staleness (version gap) for filtering examples. "
            "If current_version - min_version > max_staleness, the example will be skipped. "
            "If None, staleness filtering is disabled. "
            "If not set in buffer config, falls back to rollout.max_head_offpolicyness."
        },
    )
    replay_size: int | None = field(
        default=None,
        metadata={
            "help": "Maximum number of examples to store in the replay buffer. "
            "If None, defaults to the main buffer size."
        },
    )
    replay_ratio: float = field(
        default=0.0,
        metadata={
            "help": "Fraction of each training batch to draw from the replay buffer. "
            "Must be between 0 and 1."
        },
    )
    selection_fn: str | None = field(
        default="latest",
        metadata={
            "help": "Replay selection strategy name. Available selections: 'latest'. "
            "Defaults to 'latest'."
        },
    )
    weight_pull_interval: int = field(
        default=2,
        metadata={
            "help": "Pull new weights from WeightController every N training steps. "
            "Only used in decoupled mode (WeightController). "
            "A new weight version is pushed to RaaS when "
            "latest_version - last_pulled_version >= weight_pull_interval."
        },
    )
...
class PPOConfig(BaseExperimentConfig):
    ...
    weight_transfer_mode: str = field(
        ...
        metadata={
            "help": (
                "Weight transfer mode after each PPO step. "
                "'tcp' (default): non-blocking TCP transfer via sender_agent — "
                "RaaS pulls weights asynchronously; training continues immediately. "
                "NCCL mode has been removed."
            )
        },
    )
    weight_transfer_strategies: str = field(
        default="full",
        metadata={
            "help": (
                "Weight transfer strategy. "
                "'full': send complete weights each step. "
                "'delta': send sparse diffs (full is always available as fallback)."
            )
        },
    )
    model_id: str | None = field(
        ...
        metadata={
            "help": (
                "Model identifier for multi-model training (e.g. 'model0', 'model1'). "
                "When set, the trainer participates in multi-model version barriers "
                "and RaaS weight updates are scoped to this model. "
                "When None (default), single-model mode is used."
            )
        },
    )
    agent_name: str = field(
        ...
        metadata={
            "help": (
                "Agent name for connecting to the AstraFlow HTTP service. "
                "Must match a key in the AstraFlow service's `agents` dict. "
                "In multi-model training, each trainer uses a distinct agent_name "
                "(e.g. 'model0', 'model1') to get its own batch and version tracking."
            )
        },
    )

    # Shared generation parameters — set once in `shared` YAML section.
    # Propagated to actor.group_size, actor.reward_norm.group_size, and
    # actor.temperature in __post_init__ if those are not explicitly set.
    n_samples: int = field(
        default=1,
        metadata={"help": "Number of samples per prompt. Propagates to actor.group_size and actor.reward_norm.group_size."},
    )
    temperature: float = field(
        default=1.0,
        metadata={"help": "Sampling temperature. Propagates to actor.temperature."},
    )
    train_batch_size: int = field(
        default=256,
        metadata={
            "help": "Number of examples (sequences) per training step. "
            "Global batch size across all data-parallel ranks. "
            "In AstraFlow v2, this is sent to the HTTP service to control batch size."
        },
    )

    def __post_init__(self):
        # Propagate shared generation params to actor config.
        # Only fill in when the actor field is still at its default (None),
        # meaning the user did not explicitly set it in the YAML.
        if self.actor is not None:
            if self.actor.group_size is None:
                self.actor.group_size = self.n_samples
            if self.actor.temperature is None:
                self.actor.temperature = self.temperature
            if self.actor.reward_norm is not None:
                if self.actor.reward_norm.group_size is None:
                    self.actor.reward_norm.group_size = self.n_samples
        # Ref model also needs temperature for logprob computation.
        if self.ref is not None:
            if self.ref.temperature is None:
                self.ref.temperature = self.temperature
...
class GRPOConfig(PPOConfig):
    ...
    offline_data_path: str = field(
        default="data-data",
        metadata={"help": "Base directory containing offline datasets."},
    )
...
def parse_cli_args(argv: list[str]):
    ...
    parser.add_argument(
        "--config", help="Path to the experiment configuration file", required=True
    )
    parser.add_argument(
        "--trainer",
        type=str,
        default=None,
        help="Trainer section key (e.g. 'trainer_model0'). Default: 'trainer'.",
    )
    ...
    trainer_dict = load_trainer_config(raw, trainer_key=trainer_key)
...
def to_structured_cfg(cfg, config_cls):
    ...
    # (e.g. shared scalars like model_path, dtype that are only used for
    # ${...} substitution and shouldn't be stored in the config object).
