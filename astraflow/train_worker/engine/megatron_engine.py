...
from astraflow.train_worker.api.alloc_mode import MegatronParallelStrategy, ParallelStrategy
from astraflow.train_worker.api.cli_args import MicroBatchSpec, TrainEngineConfig
...
from astraflow.train_worker.models.mcore.hf_load import (
    load_weights_from_hf_with_mbridge_fast,
)
from astraflow.train_worker.models.mcore.hf_save import save_weights_to_hf_with_mbridge_fast
from astraflow.train_worker.models.mcore.registry import (
...
from astraflow.train_worker.utils.functional import gather_logprobs, gather_logprobs_entropy
from astraflow.train_worker.utils.hf_utils import load_hf_tokenizer
from astraflow.train_worker.utils.lock import DistributedLock
from astraflow.train_worker.utils.mcore.determinisitc import set_deterministic_algorithms
from astraflow.train_worker.utils.mcore.packed_context_parallel import (
...
from astraflow.train_worker.utils.perf_tracer import trace_scope
from astraflow.train_worker.utils.seeding import get_seed
...
class MegatronEngine(TrainEngine):
    ...
    def initialize(self, addr: str | None, ft_spec: FinetuneSpec):
        ...
        # Use flash attention backend to avoid TE fused attention's
        # libcudart version conflict when multiple CUDA toolkits coexist.
        from megatron.core.models.common.language_module.language_module import AttnBackend
        self.tf_config.attention_backend = AttnBackend.flash

        # Override sequence_parallel from bridge config when TE is not available.
        if not self.parallel_strategy.use_sequence_parallel and self.tf_config.sequence_parallel:
            self.logger.info(
                "Overriding tf_config.sequence_parallel=False "
                "(TE not available or SP disabled in parallel strategy)."
            )
            self.tf_config.sequence_parallel = False
    ...
    def _make_parallel_strategy(
        ...
        # Sequence parallelism requires Transformer Engine for fused LayerNorm.
        # Disable SP when TE is not available.
        use_sp = parallel_strategy.tensor_parallel_size > 1
        if use_sp:
            try:
                ...
                _logger.warning(
                    "Transformer Engine not available — disabling sequence parallelism. "
                    "Install TE for better memory efficiency with TP>1."
                )
                use_sp = False
    ...
    def _init_context_and_model_parallel_group(self) -> None:
        # Initialize context and model parallel groups used for data distribution
