...
from astraflow.train_worker.api.alloc_mode import FSDPParallelStrategy, ParallelStrategy
...
from astraflow.train_worker.engine.core import (
    aggregate_eval_losses,
    compute_total_loss_weight,
    reorder_and_pad_outputs,
)
from astraflow.train_worker.models.transformers.ulyssess_patch import apply_monkey_patch
from astraflow.train_worker.platforms import current_platform
from astraflow.train_worker.utils import (
    logging,
    pkg_version,
    stats_tracker,
)
from astraflow.train_worker.utils.constants import DIST_GROUP_DEFAULT_TIMEOUT
...
from astraflow.train_worker.utils.distributed import (
    patch_dist_group_timeout,
)
from astraflow.train_worker.utils.fsdp import (
    fsdp2_load_full_state_dict,
    get_cosine_schedule_with_warmup,
)
from astraflow.train_worker.utils.fsdp.checkpoint import DCPState, LoRADCPState
from astraflow.train_worker.utils.fsdp.grad import fsdp2_clip_grad_norm
from astraflow.train_worker.utils.fsdp.optimizer import AnyPrecisionAdamW
from astraflow.train_worker.utils.fsdp.parallel import ParallelHelper, parallelize_model
from astraflow.train_worker.utils.functional import (
    gather_logprobs,
    gather_logprobs_entropy,
)
from astraflow.train_worker.utils.hf_utils import (
...
from astraflow.train_worker.utils.perf_tracer import trace_perf, trace_scope
from astraflow.train_worker.utils.save_load import get_state_dict_from_repo_id_or_path
from astraflow.train_worker.utils.ulysses import (
...
class FSDPEngine(TrainEngine):
    ...
    def create_process_group(self, parallel_strategy: ParallelStrategy | None = None):
        ...
        self.dp_group = self.parallel_helper.dp_group
        self.sp_group = self.parallel_helper.sp_group
    ...
    def initialize(
        ...
        if pkg_version.is_version_less("torch", "2.4.0"):
            raise RuntimeError("Only FSDP2 is supported, which requires torch>=2.4.0")
    ...
    def update_weights(self, meta: WeightUpdateMeta):
        raise NotImplementedError(
            "NCCL/disk weight transfer has been removed. "
            "Use TCP-based weight transfer via WeightManager instead."
        )

    def transfer_weights(self, meta: WeightUpdateMeta):
        """Transfer weights to inference without pause/resume.

        Use this when an external orchestrator (e.g., AstraFlow HTTP
        service) handles the pause/resume lifecycle.
        """
        ...
        with tms_context:
            self._transfer_weights_from_distributed(meta)
    ...
    def _check_rollout_engine_connected(self) -> None:
        ...
        if self.rollout_engine is None:
            raise RuntimeError(
                ...
                " before using update_weight methods."
            )
    ...
    def _update_bucket_weights_from_distributed(
        ...
        for handle in handles:
            self._wait_dist_work(handle, "weight_broadcast")

        self._wait_weight_update_future(fut, "update_weights_from_distributed")

        named_tensors.clear()

    def _weight_update_wait_timeout_sec(self) -> float:
        """Best-effort timeout for remote weight-update control RPCs."""
        request_timeout = None
        rollout_engine = self.rollout_engine
        if rollout_engine is not None:
            rollout_config = getattr(rollout_engine, "config", None)
            if rollout_config is not None:
                request_timeout = getattr(rollout_config, "request_timeout", None)

        try:
            timeout = float(request_timeout) if request_timeout is not None else 0.0
        except (TypeError, ValueError):
            timeout = 0.0

        # Keep generous headroom, but never wait forever on control futures.
        return max(300.0, timeout * 3.0) if timeout > 0 else 900.0

    def _wait_weight_update_future(self, fut: Future, op_name: str) -> Any:
        timeout = self._weight_update_wait_timeout_sec()
        try:
            return fut.result(timeout=timeout)
        except FutureTimeoutError as exc:
            raise TimeoutError(
                f"{op_name} did not finish within {timeout:.1f}s."
            ) from exc

    def _wait_dist_work(self, work: dist.Work, op_name: str) -> None:
        timeout = self._weight_update_wait_timeout_sec()
        deadline = time.monotonic() + timeout
        # Avoid unbounded wait on distributed collectives.
        while hasattr(work, "is_completed") and not work.is_completed():
            if time.monotonic() >= deadline:
                raise TimeoutError(f"{op_name} did not finish within {timeout:.1f}s.")
            time.sleep(0.05)
        work.wait()

    def _cpu_barrier_with_trace(self, stage: str) -> None:
        rank = dist.get_rank()
        timeout_s = self._weight_update_wait_timeout_sec()
        try:
            ...
            self.logger.exception(
                "XCCL barrier failed: stage=%s rank=%s",
                stage,
                rank,
            )
            raise

    def save_weights_to_disk(self, path: str) -> None:
        """Save model weights to disk without interacting with the inference engine.

        All ranks must call this. Uses the same FSDP2 state-dict gather +
        ``save_pretrained`` path as ``_save_model_to_hf``. No ``name_resolve``,
        no ``rollout_engine`` calls.
        """
        self._save_model_to_hf(path, self.tokenizer, self.processor)

    @trace_perf("fsdp_engine.transfer_weights_from_distributed", category="comm")
    def _transfer_weights_from_distributed(self, meta: WeightUpdateMeta):
        """Broadcast parameters (chunked) without pause/resume.

        Same as ``_update_weights_from_distributed`` but does NOT call
        ``pause_generation()`` / ``continue_generation()`` on the rollout
        engine.  Use when an external orchestrator handles the inference
        lifecycle.
        """
        is_main_rank = dist.get_rank() == 0

        self._cpu_barrier_with_trace("pre_transfer")
        ...
        try:
            ...
            for name, param in param_iterator:
                ...
                if tensor_size + buffer_size > weight_chunked_mem_size:
                    self._update_bucket_weights_from_distributed(
                        meta, named_tensors, chunk_index=chunk_index
                    )
                    chunk_index += 1
                    buffer_size = 0
            ...
            self._cpu_barrier_with_trace("post_transfer")
            current_platform.synchronize()
            self._cpu_barrier_with_trace("final_sync")
            raise exc
    ...
    def _save_to_dcp(
        ...
        # Free reserved-but-unused GPU memory before DCP save so that
        # DCP's internal NCCL collectives can allocate communication buffers.
        _t0 = _time.monotonic()
        torch.cuda.empty_cache()
        _mem = torch.cuda.memory_allocated() / 1e9
        _mem_res = torch.cuda.memory_reserved() / 1e9
        print(f"[dcp_save] rank={_rank} empty_cache done, mem_alloc={_mem:.1f}GB mem_res={_mem_res:.1f}GB", flush=True)

        opt = self.optimizer if with_optim else None
        if self.config.use_lora:
            dcp_state = LoRADCPState(self.model, opt)
        else:
            dcp_state = DCPState(self.model, opt)
        state_dict = {"dcp": dcp_state}
        _t1 = _time.monotonic()
        print(f"[dcp_save] rank={_rank} calling dcp.save(with_optim={with_optim}) ...", flush=True)
        dcp.save(state_dict, checkpoint_id=path)
        print(f"[dcp_save] rank={_rank} dcp.save done in {_time.monotonic()-_t1:.1f}s", flush=True)
        torch.cuda.empty_cache()
    ...
    def _load_from_dcp(self, path: str, with_optim: bool):
        ...
        # Release temporary GPU memory allocated by DCP to prevent
        # NCCL buffer allocation failures in subsequent operations.
        torch.cuda.empty_cache()
