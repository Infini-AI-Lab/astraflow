...
def get_minerva_math_sft_dataset(
    tokenizer=None,
    max_length: int | None = None,
):
    """
    Minerva-Math is not intended for SFT training in this setup.
    """
    raise NotImplementedError("Minerva-Math dataset not supported for SFT training.")


def download_dataset(
    offline_dir: str,
    dataset_path: str = "math-ai/minervamath",
    split: str = "test",
):
    """
...
def get_minerva_math_test_dataset(
    tokenizer=None,
    max_length: int | None = None,
    offline_dir: str | None = None,
    dataset_name: str = "minerva_math",
):
    """
    Load the math-ai/minervamath dataset and adapt it for RL-style evaluation.
    ...
    if offline_dir is not None:
        ...
        logger.info("Loading Minerva-Math dataset from offline path: %s", offline_dir)
        dataset = load_from_disk(offline_dir)
    else:
        logger.info("Loading Minerva-Math dataset from online source.")
        dataset = load_dataset(path="math-ai/minervamath", split="test")
...
if __name__ == "__main__":
    ...
    dataset = get_minerva_math_test_dataset(tokenizer=tokenizer, max_length=max_length)
