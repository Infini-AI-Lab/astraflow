...
def get_amc_2024_sft_dataset(
    tokenizer=None,
    max_length: int | None = None,
):
    """
    AMC-2024 is not intended for SFT training in this setup.
    """
    raise NotImplementedError("AMC-2024 dataset not supported for SFT training.")


def download_dataset(
    offline_dir: str,
    dataset_path: str = "rawsh/2024_AMC12",
    split: str = "train",
):
    """
...
def get_amc_2024x4_test_dataset(
    tokenizer=None,
    max_length: int | None = None,
    offline_dir: str | None = None,
    dataset_name: str = "amc_2024",
):
    """
    Load the rawsh/2024_AMC12 dataset and adapt it for RL-style evaluation.
    ...
    if offline_dir is not None:
        ...
        logger.info("Loading AMC-2024 dataset from offline path: %s", offline_dir)
        dataset = load_from_disk(offline_dir)
    else:
        logger.info("Loading AMC-2024 dataset from online source.")
        dataset = load_dataset(path="rawsh/2024_AMC12", split="train")
...
if __name__ == "__main__":
    ...
    dataset = get_amc_2024x4_test_dataset(tokenizer=tokenizer, max_length=max_length)
