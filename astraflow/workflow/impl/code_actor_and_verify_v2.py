"""Two-model code workflow v2: example-stripped testgen prompt, reason-then-emit, discrete rewards."""
...
from astraflow.workflow.utils.code_execution_mraas import (
    SINGLE_CASE_EXEC_TIMEOUT,
    call_verify_collect_all,
    extract_python_code,
)

logger = logging.getLogger("CodeActorAndVerifyV2 workflow")
...
@register_workflow("code_actor_and_verify_v2")
class CodeActorAndVerifyV2Workflow(RolloutWorkflow):
    """v2 variant: example-stripped testgen prompt, reason-then-emit, discrete rewards.

    Differences vs ``code_actor_and_verify``:

    * Test generator receives the problem text with built-in examples/sample I/O
      stripped out, preventing trivial copy-paste into generated cases.
    * Test generator system prompt asks for ``<analysis>...</analysis>`` reasoning
      followed by a fenced ```json block; parser already prefers fenced blocks.
    * Code-generator reward is binary: ``1.0`` iff the attempt passes all GT
      cases, else ``0.0`` (both attempt 1 and attempt 2).
    * Test-generator reward is the sum of two binaries:
      ``int(final_code.all_passed) + int(reference_case_eval.all_passed)`` ∈
      {0, 1, 2}.
    """
    ...
    async def _run_one(
        ...
        if self.discrete_testgen_reward:
            testcase_reward = float(final_code_eval["all_passed"]) + float(
                reference_case_all_passed
            )
        else:
