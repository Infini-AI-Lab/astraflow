# Note: adapted from https://github.com/inclusionAI/AReaL
import ast
import faulthandler
import json
import platform
...
DEFAULT_MEMORY_LIMIT_BYTES = 5 * 1024 * 1024 * 1024
...
def _normalize_call_based_case(inputs, output):
    raw_inputs = inputs
    raw_outputs = output
    ...
    if isinstance(inputs, str):
        ...
        raw_inputs = truncatefn(json.dumps(raw_inputs, ensure_ascii=False), 200)
    ...
    if isinstance(output, str):
        ...
        raw_outputs = truncatefn(json.dumps(raw_outputs, ensure_ascii=False), 200)

    return parsed_inputs, parsed_output, raw_inputs, raw_outputs
...
def reliability_guard(maximum_memory_bytes=DEFAULT_MEMORY_LIMIT_BYTES):
    """
...
if __name__ == "__main__":
    ...
    if args.case_index is not None:
        try:
            io = json.loads(input_data["sample"]["input_output"])
            io["inputs"] = [io["inputs"][args.case_index]]
            io["outputs"] = [io["outputs"][args.case_index]]
            input_data["sample"]["input_output"] = json.dumps(io)
        except Exception as e:
            saved_result = {"result": [-4], "info": {"error": f"slice failed: {e!r}"}}
            with open(
            ...
            sys.exit(0)
        out_suffix = f"-{args.case_index}"

    result, info = run_test(**input_data)
    saved_result = {"result": result, "info": info}
    with open(
        f"/tmp/{args.tmp_id}{out_suffix}-output.json", "w", encoding="utf-8"
    ) as temp_file:
        json.dump(saved_result, temp_file)
